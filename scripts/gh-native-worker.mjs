...
async function start(backend) {
  ...
  primary = spawn(binary, [], { cwd: serverHome, env: { ...env, PORT: "6666", DB_DSN: "sqlite:" + join(serverHome, "ags.sqlite") + "?_foreign_keys=on", GIT_REPO_DIR: join(serverHome, "repos"), BASE_URL: "https://127.0.0.1", ADMIN_LOGIN: "fixture", ADMIN_TOKEN: token, LISTEN_MODE: "production", ENVIRONMENT: "development", ENABLE_WORKFLOW_EXEC: "0", AGS_INTEGRATIONS_CONFIG: join(work, "integrations.yaml") }, stdio: ["ignore", "pipe", "pipe"], detached: true });
