...
func NewCreateContext(opts *CreateOptions) (*CreateContext, error) {
	...
	// Check each remote individually for the current branch. We verify refs one
	// remote at a time because git show-ref --verify stops processing after it
	// hits a non-existent ref, which prevents later remotes from being checked.
	headRefs, _ := opts.GitClient.ShowRefs(context.Background(), []string{"HEAD"})
	if len(headRefs) > 0 {
		headRef := headRefs[0]
		for _, remote := range remotes {
			refName := fmt.Sprintf("refs/remotes/%s/%s", remote.Name, currentBranch)
			remoteRefs, _ := opts.GitClient.ShowRefs(context.Background(), []string{refName})
			if len(remoteRefs) > 0 && remoteRefs[0].Hash == headRef.Hash {
				remoteTrackingRef, err := git.ParseRemoteTrackingRef(remoteRefs[0].Name)
				if err != nil {
					return nil, err
				}

				qualifiedHeadRef := shared.NewQualifiedHeadRefWithoutOwner(remoteTrackingRef.Branch)
				if baseRepo.RepoOwner() != remote.RepoOwner() {
					qualifiedHeadRef = shared.NewQualifiedHeadRef(remote.RepoOwner(), remoteTrackingRef.Branch)
				}
